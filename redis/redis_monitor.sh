...
# redis自定义安装的目录
export BASE_DIR=/data/redis_data/9379

export PASSWD="0f649985e1ae10a"
...
# 密码判断和获取
if [ "$PASSWD" == "" ];then
    if [ "$1" == "9379" ] && [ -f ${BASE_DIR}/redis.conf ];then
...
# 定义命令路径
BASE_DIR1=/data/apps/redis
export REDISCLI="${BASE_DIR1}/bin/redis-cli"

if [ ! -f "$REDISCLI" ];then
    REDISCLI="${BASE_DIR1}/redis-cli"
fi
